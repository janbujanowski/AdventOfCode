...
namespace AdventOfCode2017
{
    public class Day4
    {
        ...
        private static List<List<string>> validLines = new List<List<string>>();
        public static string Input
        {
            get
            {
                return @"bdwdjjo avricm cjbmj ran lmfsom ivsof
...
jwfm ptjwrbl hhuv uolz adyweh qpj wxyogp igvnojq jmfw pqs fsnirby";
        ...
        private static void Init()
        {
            ...
            lines = Input.Split(separators, StringSplitOptions.None);
        }
        public static int StarOne()
        {
            Init();
            List<string> words;
            validLines = new List<List<string>>();
        ...
        {
            StarOne();
            List<List<string>> validExtendedSecurity = new List<List<string>>();
            foreach (var line in validLines)
            {
                var copy = new List<string>(line);
                var valid = true;
                foreach (var word in line)
                {
                    copy.Remove(word);
                    foreach (var word2 in copy)
                    {
                        //IF is anagram
                        var a = String.Concat(word.OrderBy(letter => letter));
                        var b = String.Concat(word2.OrderBy(letter => letter));
                        if (a == b)
                        {
                            valid = false;
                        }
                ...
                if (valid)
                    validExtendedSecurity.Add(line);
            }
            return validExtendedSecurity.Count;
        }
    }
}
